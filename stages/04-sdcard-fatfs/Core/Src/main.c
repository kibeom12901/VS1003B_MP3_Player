...
static void SD_SPI2_Check(void)
{
  ...
  if (r1 == 0x01) {
    ...
    r7[0] = r1;
    for (int i = 1; i < 5; i++) r7[i] = spi_xchg(0xFF);
...
int main(void)
{
  ...
  if (res == FR_OK) {
    const char *msg = "Hello from STM32 FatFS! This is Brian Kim\r\n";
    f_write(&file, msg, strlen(msg), &bw);
    f_close(&file);
    uprintln("Wrote %u bytes", bw);
  } else uprintln("f_open write fail: %d", res);
  ...
  if (res == FR_OK) {
    ...
    f_close(&file);
    uprintln("Read %u bytes: %s", br, buffer);
  } else uprintln("f_open read fail: %d", res);

  /* --- Append --- */
  res = f_open(&file, "test.txt", FA_OPEN_APPEND | FA_WRITE);
  if (res == FR_OK) {
    const char *msg2 = "Appended line.\r\n";
    f_write(&file, msg2, strlen(msg2), &bw);
    f_close(&file);
    uprintln("Appended %u bytes", bw);
  }
  ...
  uprintln("Unmounted SD");

  while (1) HAL_Delay(1000);
}

/* Keep CubeMX versions for these */
